...
import { GENERATE_SYSTEM_PROMPT, EDIT_SYSTEM_PROMPT, HTML_GENERATE_SYSTEM_PROMPT, HTML_EDIT_SYSTEM_PROMPT } from './prompts.js';
 ...
 */
export async function generateProject({ prompt, providerId, model, contextFiles = [], mode = 'react' }) {
  const contextBlock = buildContextBlock(contextFiles);
  const messages = [
    { role: 'system', content: mode === 'html' ? HTML_GENERATE_SYSTEM_PROMPT : GENERATE_SYSTEM_PROMPT },
    { role: 'user', content: prompt + contextBlock },
  ];
  const result = await callRelay({ providerId, model, messages });
  return { ...result, needsBackend: mode === 'html' ? false : result.needsBackend || likelyNeedsBackend(prompt) };
}

/**
 * Applies a chat-driven edit to an existing project.
 * @param {{instruction: string, providerId: string, model?: string, currentFiles: Array<{path:string, content:string}>, mode?: 'react'|'html'}} args
 */
export async function editProject({ instruction, providerId, model, currentFiles, mode = 'react' }) {
  const projectSnapshot = currentFiles.map((f) => `--- ${f.path} ---\n${f.content}`).join('\n\n');
  const messages = [
    { role: 'system', content: mode === 'html' ? HTML_EDIT_SYSTEM_PROMPT : EDIT_SYSTEM_PROMPT },
