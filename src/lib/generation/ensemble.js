 ...
 *
 * Jasmine and most tools in this category commit to a single model's first
 * attempt. Under BYOK, a user configuring more than one provider key isn't
 ...
 */
export async function generateEnsemble({ prompt, contextFiles = [], judgeWithSandbox }) {
  ...
  const attempts = await Promise.allSettled(
    providers.map(async (providerId) => {
      const result = await generateProject({ prompt, providerId, contextFiles });
